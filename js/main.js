...
let adddateinput = document.getElementById("due-date");
let addtaskbtn = document.getElementById("addtaskbtn");

addtaskbtn.addEventListener("click", function () {
  addtaskinputval = addtaskinput.value;
  adddateinputval = adddateinput.value;
  if (addtaskinputval.trim() != 0 && adddateinputval.trim() != 0) {
...
function edittask(index) {
  ...
  adddateinput.value = taskObj[index]["due_date"];
...
savetaskbtn.addEventListener("click", function () {
  ...
  for (keys in taskObj[saveindex]) {
    if (keys == "task_name") {
      ...
      taskObj[saveindex].due_date = adddateinput.value;
    }
  }
...
sort.addEventListener("click", function sortElementsByName() {
  const todoList = JSON.parse(localStorage.getItem("localtask"));
  var manyTodos = todoList.sort(function (a, b) {
    var x = a.task_name.toLowerCase();
    var y = b.task_name.toLowerCase();
    if (x < y) {
      return -1;
    }
    if (x > y) {
  ...
  localStorage.setItem("localtask", JSON.stringify(manyTodos));
  showtask();
});
